...
class DbSql(object):
    ...
    def connectDataBase(dbName):
        ...
        # 创建连接
        """
        默认一个线程创建的connect只能这个线程用。把check_same_thread设置为True就没这个问题了。
        参见：https://blog.csdn.net/qq_31445217/article/details/107955952
        """
        DbSql.dbConnect = sqlite3.connect(dbName, check_same_thread=False)
        # 创建游标
    ...
    def ensureTable(tableName, tableStructureInDict=None, tableStructureInStr=None, updateStrategy='rewrite'):
        """
        在全局变量dbConnect和dbCursor所保存的数据库中，按照策略建表。
                如果要建的表不存在，则直接建表。
        ...
        :param tableName: 要建立的表的名字。
        :type tableName: str
        :param tableStructureInDict: 以字典类型给出要建立的表的结构。
        :type tableStructureInDict: dict | None
        :param tableStructureInStr: 以字符串类型给出要建立的表的结构。
        :type tableStructureInStr: str | None
